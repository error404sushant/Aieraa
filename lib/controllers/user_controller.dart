...
class UserController extends GetxController implements GetxService {
  ...
  Future<ResponseModel> getUserInfo() async {
    ...
    if (response.statusCode == 200) {
      _userInfoModel = UserInfoModel.fromJson(response.body['record']);
  ...
  Future<ResponseModel> resetcurrentpass(
      String oldpass, String password, String email) async {
    _isLoading = true;
    update();
    Response response =
        await userRepo.resetcurrentpass(oldpass, password, email);
  ...
  Future<ResponseModel> updateUserImage(String files, int id) async {
    _isLoading = true;
    update();
    Response response = await userRepo.updateUserImage(files, id);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      // await authRepo.updateToken();
      if (response.body['status'] == true) {
        // _userInfoModel = UserInfoModel.fromJson(response.body['myProfile']);
        responseModel = ResponseModel(true, response.body['message']);
      } else {
        responseModel = ResponseModel(false, response.body['message']);
      }
  ...
  Future<ResponseModel> updateUser(UserInfoModel userInfo) async {
    _isLoading = true;
    update();
    Response response = await userRepo.updateUser(userInfo);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      if (response.body['status'] == true) {
        _userInfoModel = UserInfoModel.fromJson(response.body['myProfile']);
